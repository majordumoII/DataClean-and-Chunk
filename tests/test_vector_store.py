"""Tests for VectorStore (with mocked psycopg2 and pgvector)."""
...
def mock_conn():
    ...
    # mogrify must return actual bytes for execute_values to work
    cursor.mogrify.return_value = b"(%s,%s,%s,%s,%s,%s)"
...
class TestVectorStore:
    ...
    def test_stored_chunks_use_vector_cast(self, config, mocker, mock_conn):
        ...
        # execute_values calls mogrify with the template containing ::vector
        mogrify_calls = mock_conn.cursor.return_value.__enter__.return_value.mogrify.call_args_list
        template = mogrify_calls[0][0][0]
        assert "%s::vector" in template
